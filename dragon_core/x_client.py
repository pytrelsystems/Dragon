...
class XClient:
    ...
    def recent_search(self, query: str, since_id: Optional[str] = None, max_results: int = 10) -> Dict[str, Any]:
        """
        GET /2/tweets/search/recent?query=... (last 7 days)
        """
        q = (query or "").strip()
        if not q:
        ...
        if since_id:
            qs["since_id"] = since_id
        return self._get_json(f"/2/tweets/search/recent?{urllib.parse.urlencode(qs)}")
